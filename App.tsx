    ...
    try {
      ...
      const flattenedDevices = filteredData.flatMap(category => 
        category.devices.map(device => ({
          ...
          imageUrl: `https://source.unsplash.com/400x300/?${encodeURIComponent(device.imageQuery)}&${Date.now()}`
        }))
      );
  ...
  const renderContent = () => {
    ...
    if (view === 'details') {
        return isLoading ? <Loader message={getLoadingMessage()} /> : 
               deviceDetails && <DeviceDetailView details={deviceDetails} onBack={handleGoBack} />;
    }
